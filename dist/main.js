...
const scanDirectoryForFiles = (dir, baseName, maxDepth = 3) => {
    const results = [];
    const MAX_FILES = 150; // Limit to prevent UI overload
    const scan = (currentDir, depth) => {
        if (depth > maxDepth || results.length >= MAX_FILES)
            return;
        try {
            ...
            for (const entry of entries) {
                ...
                try {
                    ...
                    if (entry.isDirectory() && depth < maxDepth) {
                        ...
                        if (dirNameLower.includes(baseName) ||
                            dirNameLower === 'config' ||
                            dirNameLower === 'data' ||
                            dirNameLower === 'temp' ||
                            dirNameLower === 'cache' ||
...
const scanRegistryForProgram = async (exePath) => {
            ...
            if (stdout) {
                ...
                for (const line of lines) {
                    ...
                    if (trimmed.includes('REG_') && currentKey) {
                        ...
                        if (trimmed.toLowerCase().includes(exeName) ||
...
const deleteRegistryKeys = async (registryPaths) => {
    const results = [];
...
electron_1.ipcMain.handle('uninstaller-scan-exe', async (_, exePath) => {
    try {
...
electron_1.ipcMain.handle('uninstaller-delete', async (_, filePaths, registryPaths) => {
